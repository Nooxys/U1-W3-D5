...
console.log(splitMe('Shiba inu is big'))
/* ESERCIZIO 4
...
console.log(deleteOne('cane', true))
...
const onlyLetters = function (string) {
  ...
  //Nb: non torna la stringa completa " i have dogs", bensì torna la stringa splittata - chiedere a Stefano
}
...
console.log(isThisAnEmail('ciro.dasjdaisjda@gmail.com'))
/* ESERCIZIO 7
...
//   let sum = 0
//   let values = []
//   let result = { somma: 0, values: [] }
//   // questo nasce undefined
//   for (let i = 0; i < number; i++) {
//     let aCaso = dice()
//     sum += aCaso
//     result.values.push(aCaso)
//   }
//   result.somma = sum
//   return result
...
*/
console.log('------------------')
const newDate = new Date('march 8, 2002')

const howManyDays = function (data) {
  const oggi = new Date()
  let difference = Math.floor((oggi - data) / (1000 * 60 * 60 * 24))
  return difference
}
...
const isTodayMyBirthday = function (date) {
  const mioCompl = new Date('january 13, 1994')
  if (
    mioCompl.getDate() === date.getDate() &&
    mioCompl.getMonth() === date.getMonth()
  ) {
    return true
  } else {
    return false
...
*/
console.log('------------------')
const persona = { occhi: 'blu', capelli: 'castani', statura: 'immenso' }
const deleteProp = function (oggetto, stringa) {
  delete oggetto[stringa]
}
deleteProp(persona, 'statura')
console.log(persona)
...
const searchByTitle = function (string) {
  const films = movies.filter((movie) => movie.Title.includes(string))
  return films
}
...
searchAndDivide('Avengers', movies)
console.log(divided)
/* ESERCIZIO 19
...
const removeIndex = function (numero) {
  const newnewArr = movies.splice(numero, 1)
  return movies
}
...
*/
console.log('------------------')
const allRed = function () {
  const allLinks = document.getElementsByTagName('a')
  for (let i = 0; i < allLinks.length; i++) {
    allLinks[i].style['background-color'] = 'red'
...
const newList = function () {
  const nonOrdinata = document.getElementById('myList')
  const listina = document.createElement('li')
  listina.innerText = 'idk'
  nonOrdinata.appendChild(listina)
}
newList()
/* ESERCIZIO 25
...
*/
console.log('------------------')
const svuotamento = function () {
  const misonoRicordato = document.getElementById('myList')
  misonoRicordato.innerHTML = ''
}
svuotamento()
